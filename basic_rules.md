...
Nagrodą w przygodzie często jest jeden lub więcej magicznych przedmiotów. W OPK magiczne przedmioty dzielimy na:

a)    Przedmioty jednorazowe lub jednorazowego użytku (popularnie zwane consumable, acz to pojęcie budzi czasem nieporozumienia, więc nie będzie odtąd używane w zasadach). Za takie przedmioty uznajemy przedmioty, które tracą na zawsze swoje magiczne własności po jednokrotnym zastosowaniu (mikstury, zwoje, niektóre tatuaże, elemental gems itp.), magiczną amunicję (niezależnie od jej ilości podanej w przygodzie), czyli bełty, strzały i pociski do broni palnej oraz smokepowder oraz szczególne przedmioty o ograniczonej liczbie użyć, specyficzne dla konkretnych linii fabularnych (czyli sezonów) AL, których dołączenie do przedmiotów jednorazowych musi być zawsze przegłosowane (ich obecna lista to: Soul Coin, Balloon Pack, Wingwear).

b)    Przedmioty permanentne: wszystkie  te, które nie są jednorazowego użytku. Zwracamy uwagę, że przedmioty o skończonej, ale większej niż 1 liczbie użyć (np. Keoghtom’s Ointment, Gem of Brightness, Ring of Three Wishes) są przedmiotami permanentnymi na potrzeby tego podziału.

Ponadto, przedmioty można podzielić ze względu na rzadkość (common, uncommon, rare, very rare, legendary) zgodnie z ogólnymi zasadami DnD 5e. 

Podziały te mają znaczenie przede wszystkim przy przydzielaniu przedmiotów magicznych jako nagród po sesji (poniżej) oraz możliwość wymiany magicznych przedmiotów [(Podrozdział 4.3 Zasad Podstawowych, pierwsze 2 punkty – wymieniać można tylko przedmioty permanentne).](#downtime)

Podziałem magicznych przedmiotów jako nagród za przygodę między postacie graczy zarządza Mistrz Gry, kierując się zasadami szczegółowo opisanymi w rozdziale [3.2 Zasad Zaawansowanych](advanced_rules.md#zaawansowane-przydzielanie-przedmiotow). Z perspektywy gracza wystarczy wiedzieć, że pierwszeństwo w uzyskaniu przedmiotu mają postacie, które mogą danego przedmiotu używać, a w drugiej kolejności, w wypadku permanentnych magicznych przedmiotów o rzadkości co najmniej uncommon, pierwszeństwo w otrzymaniu takiego przedmiotu mają postacie, które posiadają takich przedmiotów mniej, niezależnie czy te przedmioty zostały zabrane na przygodę, czy nie.

Dlatego należy pamiętać o liczbie permanentnych przedmiotów magicznych o rzadkości co najmniej uncommon przypisanych do postaci uczestniczącej w przygodzie i podać tę informację  na żądanie Mistrza Gry. Zgodnie ze szczegółowymi ustaleniami do tej liczby nie wlicza się maksymalnie jeden z następujących przedmiotów: magiczna, generyczna broń +1; Wraps of Unarmed Prowess; Insignia of Claws, Uncommon. 

Podkreślamy, że przedmioty magiczne pospolite (rzadkości common), przedmioty magiczne jednorazowe i przedmioty niemagiczne (jeśli przygoda przewiduje możliwość ich zatrzymania, zamiast sprzedaży) nie wliczają się do ilości permanentnych przedmiotów magicznych i przy ich przydzielaniu jako nagród rzeczowych stan posiadania nie jest istotny.

Uwaga! Magicznych przedmiotów, za wyjątkiem tych, które zgodnie z zasadami DnD mogą utracić swoją moc (np. kostury, różdżki, przedmioty jednorazowe i z ograniczoną liczbą ładunków), nie da się po prostu usunąć ze stanu posiadania postaci. Nie sposób ich trwale zniszczyć, wyrzucić, sprzedać za pieniądze ani w inny sposób się pozbyć, zmniejszając liczbę posiadanych permanentnych magicznych przedmiotów, chyba że przygoda wprost takie usunięcie przewiduje. Dopuszczalna jest jedynie wymiana (lub w wypadku broni: przekucie) na inne magiczne przedmioty przeprowadzona w sposób opisany w [Rozdziale 4.4 Zasad Podstawowych.](#przekuwanie-broni)
...
**2. Wymian permanentnych magicznych przedmiotów z listą magicznych przedmiotów na wymianę:** Wymiany permanentnych magicznych przedmiotów można też dokonać ze słynnym paserem S.Wawelskim w lombardzie ,,Smocza Jama''. Taka wymiana nie wymaga zgody innych graczy i można w ten sposób wymienić również przedmiot o rzadkości wyższej na przedmiot o rzadkości niższej (np. rare na uncommon), ale lista przedmiotów, które można uzyskać jest znacząco krótsza, a w niektórych przypadkach koszt jest wyższy. Konkretne ceny, lista przedmiotów, które można w zamian uzyskać i ograniczenia takich wymian znajdują się w [Tabeli 5 części II tych zasad](#tabela-lombard). Przedmioty oddane do lombardu w ramach wymiany znikają w smoczym skarbcu i nie zostaną nigdy odzyskane.
